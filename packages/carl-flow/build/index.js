...
var Store = /** @class */ (function () {
    function Store(models, middlewares) {
        ...
        this.watchers = [];
        // 快照构建
        this.currentTree = models.reduce(function (prev, _a) {
        ...
        this.serivces = models.reduce(function (prev, _a) {
            var namespace = _a.namespace, services = _a.services;
            return Object.entries(services).reduce(function (tree, _a) {
                var key = _a[0], serivce = _a[1];
                tree[namespace + "/" + key] = serivce;
                return tree;
            }, prev);
        }, {});
        this.tree$ = new BehaviorSubject(this.currentTree);
        this.subscription = this.tree$.subscribe(function (next) {
            // 事件监听
            _this.watchers.forEach(function (watcher) {
                watcher(_this.currentTree, next);
            });
            // 这种严格来讲不算插件机制，暂时先这么处理
            if (middlewares.length > 0) {
                var firstMiddleware = middlewares[0];
                var firstPromise = firstMiddleware(_this, next, _this.currentTree);
                if (middlewares.length > 0) {
                    var Promised = middlewares.slice(1).reduce(function (wrap, fn) { return wrap.then(function (tree) { return fn(_this, next, tree); }); }, firstPromise);
                    Promised.then(function (tree) { return _this.currentTree = tree; });
                }
                else {
                    firstPromise.then(function (tree) { return _this.currentTree = tree; });
                }
            }
            else {
                _this.currentTree = next;
            }
        });
        this.alive = true;
    }
    Store.prototype.dispatch = function (action) {
        ...
        if (isObservable(action)) {
            ...
            var currentNamespace_1 = type.split('/')[0];
            var service = this.serivces[type];
            if (service) {
                return service(payload, {
                    next: this.dispatch,
                    select: this.select,
                    put: function (payload, namespace) {
                        return _this.put(namespace || currentNamespace_1, payload);
                    },
                });
            }
    ...
    Store.prototype.dispath$ = function (action$) {
        var _this = this;
        if (!this.alive) {
            throw new Error('Store is destoryed, never call this function againt');
        }
        return new Promise(function (resolve, reject) {
            var caches = [];
            var subscription = action$.subscribe(function (action) {
                _this.dispatch(action)
                    .then(function (data) { return caches.push(data); })
                    .catch(reject);
            }, function (err) {
    ...
    Store.prototype.put = function (namespace, payload) {
        if (!this.alive) {
            throw new Error('Store is destoryed, never call this function againt');
        }
        // TODO 树的刷新，这里会重新构造一个树，可能会有性能问题
        var nextTree = Object.entries(this.currentTree).reduce(function (tree, _a) {
            var key = _a[0], state = _a[1];
            if (key === namespace) {
                tree[key] = Object.entries(payload).reduce(function (prev, _a) {
    ...
    // 监听
    Store.prototype.watch = function (onChange) {
        if (!this.alive) {
            throw new Error('Store is destoryed, never call this function againt');
        }
        this.watchers.push(onChange);
    };
    Store.prototype.getCurrentTree = function () {
        return this.currentTree;
    };
    // 订阅内部数据
    // 指定namespace时为特定命名空间，否则返回整个树
    Store.prototype.subscribe = function (watcher, namespace) {
        if (!this.alive) {
            throw new Error('Store is destoryed, never call this function againt');
        }
        return namespace
    ...
    // 取消订阅
    // 取消外部订阅
    Store.prototype.unsubscribe = function (subscription) {
        subscription.unsubscribe();
    };
    // 销毁内部订阅
...
var createStore = function (models, middlewares) {
    return new Store(models, middlewares || []);
};
...
/** 允许对外分发 */
var dispatch = (function (store) { return function (action) {
    return store.dispatch(action).then(function (data) { return data; });
}; });

/** 处理数据更新 */
var update = (function (store) { return function (namespace, payload) {
    return store.put(namespace, payload);
}; });

var Provider = getContext().Provider;
var Provider$1 = (function (props) { return (react.createElement(Provider, { value: props.store }, props.children)); });
 ...
 */
var Flow = function (mapTreeStreamToProps) {
    return function (InnerComp) {
        var Consumer = function (props) { return (react.createElement(Context.Consumer, null, function (context) { return (react.createElement(Stream, __assign({}, props, { __store: context }), props.children)); })); };
        var Stream = /** @class */ (function (_super) {
            __extends(Stream, _super);
            function Stream(props) {
                var _this = _super.call(this, props) || this;
                _this.state = {};
                var __store = _this.props.__store;
                if (__store) {
                    var store = __store;
                    _this.stream$ = new BehaviorSubject(store.getCurrentTree());
                    // 订阅store
                    _this.subscription = store.subscribe(function (next) { return _this.stream$.next(next); });
                    // 映射
                    if (mapTreeStreamToProps) {
                        var init_1 = true;
                        _this.stateSubscription = mapTreeStreamToProps(_this.stream$).subscribe(function (next) {
            ...
            Stream.prototype.render = function () {
                var _a = this.props, __store = _a.__store, props = __rest(_a, ["__store"]);
                return (react.createElement(InnerComp, __assign({}, props, this.state, { dispatch: __store.dispatch.bind(__store) })));
            };
            return Stream;
        }(react.Component));
...
var logger = (function (_api, next, prev) {
